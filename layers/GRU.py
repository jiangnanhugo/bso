...
class AttGRU(object):
    ...
    def build(self):
        ...
        def _recurrence(x_t, xx_t, m, h_tm1,
            ...
            # attention
            alpha = T.dot(T.tanh(T.dot(h_t, self.W_comb_att) + pctx_t), self.U_att) + self.c_tt
            ...
            # another gru cell
            preact2 = T.dot(h_t, self.U_nl) + T.dot(ctx_t, self.Wc) + self.b_nl

            # reset fate # Update gate
            r_t2 = T.nnet.sigmoid(split(preact2, 0, self.n_hidden))
            z_t2 = T.nnet.sigmoid(split(preact2, 1, self.n_hidden))
            # Cell update
            c_t2 = T.tanh((T.dot(h, self.Ux_nl) + self.bx_nl) * r_t2 + T.dot(ctx_, self.Wcx))
            # Hidden state
        ...
        h, _ = theano.scan(fn=_recurrence,
                           sequences=seqs,
                           non_sequences=[self.context, self.U, self.Ux],
                           outputs_info=self.init_state,
                           truncate_gradient=-1)
...
class BiGRU(object):
    ...
    def build_graph(self):
        ...
        ctx = T.concatenate([h[0], h[0][::-1]], axis=h[0].ndim - 1)
